...
def main(argv):
	...
	logging.info("All variants: AF + CSQ + TYPE")

	x = df.filter(['gnomAD_v2_exome_AF_popmax', 'csqCV', 'typeCV'])
	...
	logging.info("Indel and SNV: AF + CSQ")
	...
	x = df.filter(['MPC_score', 'gnomAD_v2_exome_AF_popmax', 'csqCV', 'typeCV'])
	...
	logging.info("Missense (AF) and non-missense (AF + CSQ + TYPE)")
	...
	logging.info("Missense (AF + MPC), non-missense SNV (AF + CSQ) and indel (AF + CSQ)")

	x = df.filter(['MPC_score', 'gnomAD_v2_exome_AF_popmax', 'csqCV', 'typeCV'])


	x_indel = x[ x['typeCV'] == 'indel' ]
	x_indel = x_indel.drop(['typeCV', 'MPC_score'], axis=1)
	x_indel = pd.get_dummies(x_indel, drop_first = True, columns = ['csqCV'])
	...
	x_nonMissenseSNV = x_nonMissenseSNV.drop(['MPC_score', 'typeCV'], axis=1)
